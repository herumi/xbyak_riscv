...
std::vector<int> generateRandomIntVector(size_t size, int minValue=-1000, int maxValue=1000) {
    std::random_device random_device;
    std::mt19937 random_engine(random_device());
    std::uniform_int_distribution<int> udistr(minValue, maxValue);
...
bool areIntVectorsEqual(const std::vector<int>& lhs, const std::vector<int>& rhs) {
...
void vecSumReference(int* result, const int* lhs, const int* rhs, int size) {
...
int main() {
	...
	jitKernel.ready();
	const auto vecSumJIT = jitKernel.getCode<void (*)(int*, const int*, const int*, int)>();
	...
	for (auto arrSize : testSizes) {
        ...
        std::vector<int> lhs = generateRandomIntVector(arrSize);
		std::vector<int> rhs = generateRandomIntVector(arrSize);

        // sum the vectors with the reference implementation
		std::vector<int> reference(arrSize);
		vecSumReference(reference.data(), lhs.data(), rhs.data(), arrSize);

		// sum the vectors with the generated JIT function
		std::vector<int> jitResult(arrSize);
		vecSumJIT(jitResult.data(), lhs.data(), rhs.data(), arrSize);

        // compare results
		assert(areIntVectorsEqual(reference, jitResult));
	}
