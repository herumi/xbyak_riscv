...
public:
	...
	constexpr Kind getKind() const { return static_cast<Kind>(kind_); }
	constexpr int getIdx() const { return idx_ & (EXT8BIT - 1); }
	constexpr bool isNone() const { return kind_ == 0; }
	constexpr bool isMMX() const { return is(MMX); }
	constexpr bool isXMM() const { return is(XMM); }
	constexpr bool isYMM() const { return is(YMM); }
	constexpr bool isZMM() const { return is(ZMM); }
	constexpr bool isTMM() const { return is(TMM); }
	constexpr bool isXMEM() const { return is(XMM | MEM); }
	constexpr bool isYMEM() const { return is(YMM | MEM); }
	constexpr bool isZMEM() const { return is(ZMM | MEM); }
	constexpr bool isOPMASK() const { return is(OPMASK); }
	constexpr bool isBNDREG() const { return is(BNDREG); }
	constexpr bool isREG(int bit = 0) const { return is(REG, bit); }
	constexpr bool isMEM(int bit = 0) const { return is(MEM, bit); }
	constexpr bool isFPU() const { return is(FPU); }
	constexpr bool isExt8bit() const { return (idx_ & EXT8BIT) != 0; }
	constexpr bool isExtIdx() const { return (getIdx() & 8) != 0; }
	constexpr bool isExtIdx2() const { return (getIdx() & 16) != 0; }
	constexpr bool hasEvex() const { return isZMM() || isExtIdx2() || getOpmaskIdx() || getRounding(); }
	constexpr bool hasRex() const { return isExt8bit() || isREG(64) || isExtIdx(); }
	constexpr bool hasZero() const { return zero_; }
	constexpr int getOpmaskIdx() const { return mask_; }
	constexpr int getRounding() const { return rounding_; }
	...
	{
		if (mask_) XBYAK_RISCV_THROW(ERR_OPMASK_IS_ALREADY_SET)
		mask_ = idx;
	}
	void setRounding(int idx)
	{
		if (rounding_) XBYAK_RISCV_THROW(ERR_ROUNDING_IS_ALREADY_SET)
		rounding_ = idx;
	}
	...
	constexpr bool is(int kind, uint32_t bit = 0) const
	{
		return (kind == 0 || (kind_ & kind)) && (bit == 0 || (bit_ & bit)); // cf. you can set (8|16)
	}
	constexpr bool isBit(uint32_t bit) const { return (bit_ & bit) != 0; }
...
ERR:
	XBYAK_RISCV_THROW(ERR_CANT_CONVERT)
}
...
struct Reg8 : public Reg {
	explicit constexpr Reg8(int idx = 0, bool ext8bit = false) : Reg(idx, Operand::REG, 8, ext8bit) { }
};

struct Reg16 : public Reg {
	explicit constexpr Reg16(int idx = 0) : Reg(idx, Operand::REG, 16) { }
};

struct Mmx : public Reg {
	explicit constexpr Mmx(int idx = 0, Kind kind = Operand::MMX, int bit = 64) : Reg(idx, kind, bit) { }
};
...
struct EvexModifierRounding {
	...
	explicit constexpr EvexModifierRounding(int rounding) : rounding(rounding) {}
	int rounding;
};
struct EvexModifierZero{ constexpr EvexModifierZero() {}};

struct Reg64 : public Reg32e {
	explicit constexpr Reg64(int idx = 0) : Reg32e(idx, 64) {}
};
...
#ifndef XBYAK_RISCV_DISABLE_SEGMENT
...
public:
	...
	explicit constexpr Segment(int idx) : idx_(idx) { assert(0 <= idx_ && idx_ < 6); }
...
public:
	...
	enum { i32e = 32 };
#endif
	constexpr RegExp(size_t disp = 0) : scale_(0), disp_(disp) { }
	constexpr RegExp(const Reg& r, int scale = 1)
		: scale_(scale)
		, disp_(0)
	{
		if (!r.isREG(i32e) && !r.is(Reg::XMM|Reg::YMM|Reg::ZMM|Reg::TMM)) XBYAK_RISCV_THROW(ERR_BAD_SIZE_OF_REGISTER)
		if (scale == 0) return;
		if (scale != 1 && scale != 2 && scale != 4 && scale != 8) XBYAK_RISCV_THROW(ERR_BAD_SCALE)
	...
	{
		if (base_.getBit() >= 128) XBYAK_RISCV_THROW(ERR_BAD_SIZE_OF_REGISTER)
		if (index_.getBit() && index_.getBit() <= 64) {
			if (index_.getIdx() == Operand::ESP) XBYAK_RISCV_THROW(ERR_ESP_CANT_BE_INDEX)
			if (base_.getBit() && base_.getBit() != index_.getBit()) XBYAK_RISCV_THROW(ERR_BAD_SIZE_OF_REGISTER)
		}
	}
...
{
	if (a.index_.getBit() && b.index_.getBit()) XBYAK_RISCV_THROW_RET(ERR_BAD_ADDRESSING, RegExp())
	RegExp ret = a;
	if (!ret.index_.getBit()) { ret.index_ = b.index_; ret.scale_ = b.scale_; }
	if (b.base_.getBit()) {
		if (ret.base_.getBit()) {
			if (ret.index_.getBit()) XBYAK_RISCV_THROW_RET(ERR_BAD_ADDRESSING, RegExp())
...
public:
	...
	constexpr Address(uint32_t sizeBit, bool broadcast, const RegExp& e)
		: Operand(0, MEM, sizeBit), e_(e), label_(0), mode_(M_ModRM), broadcast_(broadcast)
	{
		e_.verify();
	}
#ifdef XBYAK_RISCV64
	explicit constexpr Address(size_t disp)
		: Operand(0, MEM, 64), e_(disp), label_(0), mode_(M_64bitDisp), broadcast_(false){ }
	constexpr Address(uint32_t sizeBit, bool broadcast, const RegRip& addr)
...
public:
	...
	explicit constexpr AddressFrame(uint32_t bit, bool broadcast = false) : bit_(bit), broadcast_(broadcast) { }
...
class LabelManager {
	...
	typedef std::unordered_map<std::string, SlabelVal> SlabelDefList;
	typedef std::unordered_multimap<std::string, const JmpLabel> SlabelUndefList;
	struct SlabelState {
...
public:
	...
	{
		if (label == "@b" || label == "@f") XBYAK_RISCV_THROW(ERR_BAD_LABEL_STR)
		if (label == "@@") {
	...
	bool getOffset(size_t *offset, std::string& label) const
	{
		...
		if (label == "@b") {
			if (defList.find("@f") != defList.end()) {
				...
				XBYAK_RISCV_THROW_RET(ERR_LABEL_IS_NOT_FOUND, false)
			}
		} else if (label == "@f") {
			if (defList.find("@f") != defList.end()) {
				label = "@b";
...
private:
	...
	void rex(const Operand& op1, const Operand& op2 = Operand())
	{
		...
		if (p1->isMEM()) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
		if (p2->isMEM()) {
	...
	void vex(const Reg& reg, const Reg& base, const Operand *v, int type, int code, bool x = false)
	{
		...
		if ((idx | reg.getIdx() | base.getIdx()) >= 16) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	...
	void verifySAE(const Reg& r, int type) const
	{
		...
		XBYAK_RISCV_THROW(ERR_SAE_IS_INVALID)
	}
	...
	{
		if (!(type & (T_EVEX | T_MUST_EVEX))) XBYAK_RISCV_THROW_RET(ERR_EVEX_IS_INVALID, 0)
...
#if defined(XBYAK_RISCV64) && !defined(__ILP32__)
#ifdef XBYAK_RISCV_OLD_DISP_CHECK
		// treat 0xffffffff as 0xffffffffffffffff
		uint64_t high = disp64 >> 32;
		if (high != 0 && high != 0xFFFFFFFF) XBYAK_RISCV_THROW(ERR_OFFSET_IS_TOO_BIG)
#else
		// displacement should be a signed 32-bit value, so also check sign bit
		uint64_t high = disp64 >> 31;
		if (high != 0 && high != 0x1FFFFFFFF) XBYAK_RISCV_THROW(ERR_OFFSET_IS_TOO_BIG)
#endif
#endif
	...
	{
		if (addr.is64bitDisp()) XBYAK_RISCV_THROW(ERR_CANT_USE_64BIT_DISP)
		rex(addr, reg);
	...
	{
		if (addr.is64bitDisp()) XBYAK_RISCV_THROW(ERR_CANT_USE_64BIT_DISP)
		if (reg.isBit(8)) XBYAK_RISCV_THROW(ERR_BAD_SIZE_OF_REGISTER)
		rex(addr, reg);
	...
	{
		if (addr.is64bitDisp()) XBYAK_RISCV_THROW(ERR_CANT_USE_64BIT_DISP)
		if (addr.getMode() != Address::M_ModRM) XBYAK_RISCV_THROW(ERR_INVALID_MIB_ADDRESS)
		if (BIT == 64 && addr.is32bit()) db(0x67);
	...
	{
		if (type == T_FAR) XBYAK_RISCV_THROW(ERR_NOT_SUPPORTED)
	...
	void opJmpAbs(const void *addr, LabelType type, uint8_t shortCode, uint8_t longCode, uint8_t longPref = 0)
	{
		if (type == T_FAR) XBYAK_RISCV_THROW(ERR_NOT_SUPPORTED)
		if (isAutoGrow()) {
			if (!isNEAR(type)) XBYAK_RISCV_THROW(ERR_ONLY_T_NEAR_IS_SUPPORTED_IN_AUTO_GROW)
	...
	void opJmpOp(const Operand& op, LabelType type, int ext)
	{
		...
		if (type == T_FAR) {
			if (!op.isMEM(bit)) XBYAK_RISCV_THROW(ERR_NOT_SUPPORTED)
	...
	{
		if (!permitVisb && addr.isVsib()) XBYAK_RISCV_THROW(ERR_BAD_VSIB_ADDRESSING)
		if (addr.getMode() == Address::M_ModRM) {
			...
			if (addr.getLabel()) { // [rip + Label]
				...
				if (addr.getMode() == Address::M_ripAddr) {
					if (isAutoGrow()) XBYAK_RISCV_THROW(ERR_INVALID_RIP_IN_AUTO_GROW)
	...
	{
		if (isValid && !isValid(reg, op)) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
		if (!isValidSSE(reg) || !isValidSSE(op)) XBYAK_RISCV_THROW(ERR_NOT_SUPPORTED)
	...
	{
		if (!isValidSSE(mmx)) XBYAK_RISCV_THROW(ERR_NOT_SUPPORTED)
	...
	{
		if (!isValidSSE(op1) || !isValidSSE(op2)) XBYAK_RISCV_THROW(ERR_NOT_SUPPORTED)
		if (pref != NONE) db(pref);
		if (op1.isXMM() && op2.isMEM()) {
			...
			XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
		}
	}
	void opExt(const Operand& op, const Mmx& mmx, int code, int imm, bool hasMMX2 = false)
	{
		if (!isValidSSE(op) || !isValidSSE(mmx)) XBYAK_RISCV_THROW(ERR_NOT_SUPPORTED)
	...
	{
		if (_cl.getIdx() != Operand::CL) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	...
	{
		if (_cl && _cl->getIdx() != Operand::CL) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	...
	void opRM_I(const Operand& op, uint32_t imm, int code, int ext)
	{
		...
		if (op.getBit() < immBit) XBYAK_RISCV_THROW(ERR_IMM_IS_TOO_BIG)
	...
	{
		if (op.isMEM() && op.getBit() == 0) XBYAK_RISCV_THROW(ERR_MEM_SIZE_IS_NOT_SPECIFIED)
	}
	/*
	...
	*/
	...
	{
		if (op.isBit(32)) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
		int w = op.isBit(16);
	...
	void opFpuMem(const Address& addr, uint8_t m16, uint8_t m32, uint8_t m64, uint8_t ext, uint8_t m64ext)
	{
		...
		if (!code) XBYAK_RISCV_THROW(ERR_BAD_MEM_SIZE)
	...
	void opFpuFpu(const Fpu& reg1, const Fpu& reg2, uint32_t code1, uint32_t code2)
	{
		...
		if (!code) XBYAK_RISCV_THROW(ERR_BAD_ST_COMBINATION)
		db(uint8_t(code >> 8));
	...
	void opVex(const Reg& r, const Operand *p1, const Operand& op2, int type, int code, int imm8 = NONE)
	{
		if (op2.isMEM()) {
			...
			if ((type & (T_MUST_EVEX|T_MEM_EVEX)) || r.hasEvex() || (p1 && p1->hasEvex()) || addr.isBroadcast() || addr.getOpmaskIdx()) {
				...
				if (aaa && !(type & T_M_K)) XBYAK_RISCV_THROW(ERR_INVALID_OPMASK_WITH_MEMORY)
				bool b = false;
				if (addr.isBroadcast()) {
					if (!(type & (T_B32 | T_B64))) XBYAK_RISCV_THROW(ERR_INVALID_BROADCAST)
					b = true;
				}
	...
	void opGpr(const Reg32e& r, const Operand& op1, const Operand& op2, int type, uint8_t code, bool isR_R_RM, int imm8 = NONE)
	{
		...
		if (p1->getBit() != bit || (p2->isREG() && p2->getBit() != bit)) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	...
	void opAVX_X_X_XM(const Xmm& x1, const Operand& op1, const Operand& op2, int type, int code0, int imm8 = NONE)
	{
		...
		// (x1, x2, op)
		if (!((x1.isXMM() && x2->isXMM()) || ((type & T_YMM) && ((x1.isYMM() && x2->isYMM()) || (x1.isZMM() && x2->isZMM()))))) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	...
	{
		if (!op3.isMEM() && (x2.getKind() != op3.getKind())) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
		opVex(k, &x2, op3, type, code0, imm8);
	}
	// (x, x/m), (y, x/m256), (z, y/m)
	void checkCvt1(const Operand& x, const Operand& op) const
	{
		if (!op.isMEM() && !(x.is(Operand::XMM | Operand::YMM) && op.isXMM()) && !(x.isZMM() && op.isYMM())) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	}
	// (x, x/m), (x, y/m256), (y, z/m)
	void checkCvt2(const Xmm& x, const Operand& op) const
	{
		if (!(x.isXMM() && op.is(Operand::XMM | Operand::YMM | Operand::MEM)) && !(x.isYMM() && op.is(Operand::ZMM | Operand::MEM))) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	}
	...
	{
		if (!(x1.isXMM() && x2.isXMM() && (op.isREG(i32e) || op.isMEM()))) XBYAK_RISCV_THROW(ERR_BAD_SIZE_OF_REGISTER)
		Xmm x(op.getIdx());
	...
	{
		if (!(x.isXMM() && op.is(Operand::XMM | Operand::YMM | Operand::MEM) && op.isBit(128|256)) && !(x.isYMM() && op.is(Operand::ZMM | Operand::MEM))) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	}
	// (x, x/y/z/xword/yword/zword)
	void opCvt5(const Xmm& x, const Operand& op, int type, int code)
	{
		if (!(x.isXMM() && op.isBit(128|256|512))) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
	...
	void opSp1(const Reg& reg, const Operand& op, uint8_t pref, uint8_t code0, uint8_t code1)
	{
		...
		if (!is16bit && !(reg.isREG(i32e) && (op.isREG(reg.getBit()) || op.isMEM()))) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
		if (is16bit) db(0x66);
	...
	{
		const RegExp& regExp = addr.getRegExp();
		if (!regExp.isVsib(128 | 256)) XBYAK_RISCV_THROW(ERR_BAD_VSIB_ADDRESSING)
		const int y_vx_y = 0;
		const int y_vy_y = 1;
//		const int x_vy_x = 2;
		...
		if (!x1.isXMM() || isAddrYMM || !x2.isXMM()) {
			...
			if (!isOK) XBYAK_RISCV_THROW(ERR_BAD_VSIB_ADDRESSING)
		}
		int i1 = x1.getIdx();
		int i2 = regExp.getIndex().getIdx();
		int i3 = x2.getIdx();
		if (i1 == i2 || i1 == i3 || i2 == i3) XBYAK_RISCV_THROW(ERR_SAME_REGS_ARE_INVALID);
	...
	void checkGather2(const Xmm& x1, const Reg& x2, int mode) const
	{
		...
		XBYAK_RISCV_THROW(ERR_BAD_VSIB_ADDRESSING)
	}
	void opGather2(const Xmm& x, const Address& addr, int type, uint8_t code, int mode)
	{
		if (x.hasZero()) XBYAK_RISCV_THROW(ERR_INVALID_ZERO)
		const RegExp& regExp = addr.getRegExp();
		checkGather2(x, regExp.getIndex(), mode);
		int maskIdx = x.getOpmaskIdx();
		if ((type & T_M_K) && addr.getOpmaskIdx()) maskIdx = addr.getOpmaskIdx();
		if (maskIdx == 0) XBYAK_RISCV_THROW(ERR_K0_IS_INVALID);
		if (!(type & T_M_K) && x.getIdx() == regExp.getIndex().getIdx()) XBYAK_RISCV_THROW(ERR_SAME_REGS_ARE_INVALID);
	...
	*/
	...
	{
		if (mode) {
			if (!op.isMEM() && !((op.isXMM() && x.isXMM()) || (op.isXMM() && x.isYMM()) || (op.isYMM() && x.isZMM()))) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
		} else {
			if (!op.isMEM() && !op.isXMM()) XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
		}
		opVex(x, 0, op, type, code);
	}
	void opGatherFetch(const Address& addr, const Xmm& x, int type, uint8_t code, Operand::Kind kind)
	{
		if (addr.hasZero()) XBYAK_RISCV_THROW(ERR_INVALID_ZERO)
		if (addr.getRegExp().getIndex().getKind() != kind) XBYAK_RISCV_THROW(ERR_BAD_VSIB_ADDRESSING)
		opVex(x, 0, addr, type, code);
	}
...
#ifdef XBYAK_RISCV_DISABLE_AVX512
			XBYAK_RISCV_THROW(ERR_EVEX_IS_INVALID)
#endif
			return T_MUST_EVEX;
		}
		return 0;
	...
	void opAMX(const Tmm& t1, const Address& addr, int type, int code0)
	{
		...
		if (exp.getBase().getBit() == 0 || exp.getIndex().getBit() == 0) XBYAK_RISCV_THROW(ERR_NOT_SUPPORTED)
...
public:
	...
	*/
...
#ifdef XBYAK_RISCV_VARIADIC_TEMPLATE
	...
	void mov(const Operand& op, uint64_t imm)
	{
		if (op.isREG()) {
			...
			if (immSize <= 4) {
				...
				if (s != 0 && s != -1) XBYAK_RISCV_THROW(ERR_IMM_IS_TOO_BIG)
			} else {
				if (!inner::IsInInt32(imm)) XBYAK_RISCV_THROW(ERR_IMM_IS_TOO_BIG)
				immSize = 4;
			}
	...
	void pop(const Segment& seg)
	{
		...
		case Segment::cs: XBYAK_RISCV_THROW(ERR_BAD_COMBINATION)
...
#include "xbyak_mnemonic.h"
	...
	*/
	...
	{
		if (x == 1) return;
		if (x < 1 || (x & (x - 1))) XBYAK_RISCV_THROW(ERR_BAD_ALIGN)
...
#endif
#ifndef XBYAK_RISCV_DISABLE_SEGMENT
static const constexpr Segment es(Segment::es), cs(Segment::cs), ss(Segment::ss), ds(Segment::ds), fs(Segment::fs), gs(Segment::gs);
#endif
} // util
